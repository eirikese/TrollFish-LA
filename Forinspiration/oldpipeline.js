...
import { parseCsvTrack } from './csv-gps.js';
// import { parseGoProVideoTrack } from './gopro-gps.js';
import { matchVideoTracksToCsv } from './matcher.js';
...
export async function ingestFiles(projectId, pickedFiles, onProgress = null) {
  ...
  const processingQueue = insertedFiles
    .slice()
    .sort((a, b) => {
      const aRank = a.kind === 'csv' ? 0 : 1;
      const bRank = b.kind === 'csv' ? 0 : 1;
      return aRank - bRank;
    });

  for (const rec of processingQueue) {
    step++;
    const pct = step / totalSteps;
    report(`Processing ${rec.filename}...`, pct * 0.9);
    ...
    try {
      if (rec.kind === 'csv') {
        ...
        await processVideoFile(projectId, rec, (videoProgress) => {
          report(`Extracting GPS from ${rec.filename}...`, (pct - 1/totalSteps + videoProgress/totalSteps) * 0.9);
        });
      }
...
async function processVideoFile(projectId, fileRec, onProgress) {
  ...
  try {
    const { parseGoProVideoTrack } = await import('./gopro-gps.js');
...
export async function runMatching(projectId) {
  ...
  for (const track of allTracks) {
    ...
    const entry = { file_id: track.file_id, points };
    if (track.kind === 'video') {
  ...
  const matches = matchVideoTracksToCsv(videoTracks, csvTracks);
