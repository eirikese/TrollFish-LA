...
import * as Pipeline from './modules/pipeline.js?v=20260408iphone7';
import * as PoseEngine from './modules/pose-engine.js?v=20260430framecount1';
import * as Storage from './modules/storage.js';
import { buildReportData, generateDensityGrid } from './modules/report-builder.js?v=20260417maneuverreport1';
import { generatePdf, renderKdeHistogram } from './modules/report-pdf.js?v=20260417maneuverpdf1';
import {
  DEFAULT_MANEUVER_DETECTION_SETTINGS,
  buildManeuverAnalysis,
  detectProjectManeuvers,
  normalizeManeuverDetectionSettings,
} from './modules/maneuvers.js?v=20260430pitchheel1';
...
const state = {
  ...
  cvStatuses: {},
  cvConfig: {},
  windConfig: { disableTowFilteringTrusted: false },
  athletes: [],
  fileMeta: {},
  trackLayers: {},
  posMarkers: {},
...
const DEFAULT_WIND_CONFIG = Object.freeze({
  disableTowFilteringTrusted: false,
});
const IS_IPAD = (() => {
...
function normalizeWindConfig(config = {}) {
  return {
    disableTowFilteringTrusted: !!config?.disableTowFilteringTrusted,
  };
}
...
function guessCsvAthleteProfile(csvFilename) {
  ...
  const memory = loadCsvAthleteMemory();
  return memory.exact[fileKey] || memory.prefix[pref] || null;
}
...
function showSlotEmptyState(slot, message = 'No video at this time') {
  if (!slot?.paneEl) return;
  slot.paneEl.style.display = slot.videos?.length ? 'flex' : 'none';
  if (slot._emptyEl) {
    slot._emptyEl.textContent = message;
...
function syncPhonePlaybackShellSize() {
  ...
  const nextW = Math.max(minW, Math.min(Number(state.phonePlayback?.savedShellWidthPx) || fallbackW, maxW));
...
function updatePhonePlaybackToggleButton() {
  ...
  btn.textContent = enabled ? 'Phone/External: ON' : 'Phone/External: OFF';
  btn.title = hasMedia
    ? (enabled ? 'Turn synced phone/external playback off' : 'Turn synced phone/external playback on')
    : 'No phone/external playback clips found in this project';
}
...
async function selectProject(id) {
  ...
  clearTrackLayers();
  state.mapData = null; state.cvStatuses = {}; state.cvConfig = {}; state.windConfig = { disableTowFilteringTrusted: false }; state.athletes = []; state.fileMeta = {};
  ...
  if(!setup.hasFiles) state.wizardStep = 2;
  else if(!setup.hasAthletes) state.wizardStep = 3;
  else if(!setup.assignmentsReady) state.wizardStep = 4;
...
async function createProject(name) {
  try {
    ...
    // Switch to upload wizard step for new profiles
    state.wizardStep = 2;
...
function toggleSelectVisibleManeuvers() {
  const visible = getVisibleManeuvers();
  const ids = visible.map(move => String(move?.id || '')).filter(Boolean);
  if (!ids.length) {
    alert('No maneuvers match the current filters.');
    return;
  }
  const checked = new Set((state.maneuverChecked || []).map(String));
  const allVisibleChecked = ids.every(id => checked.has(id));
  if (allVisibleChecked) {
    for (const id of ids) checked.delete(id);
    state.maneuverChecked = [...checked];
    const remainingSelected = (state.maneuverSelected || []).map(String).filter(id => !ids.includes(id));
    const remainingChecked = (state.maneuverChecked || []).map(String).filter(Boolean);
    setManeuverSelection(remainingChecked.length ? remainingChecked : remainingSelected, {
      openWorkspace: remainingChecked.length > 0 || remainingSelected.length > 0,
  ...
  state.maneuverChecked = [...checked];
  setCompareSelectionFromChecked({ openWorkspace: true, preserveMapView: true });
}
...
function setCompareSelectionFromChecked({ openWorkspace = true, preserveMapView = true, fallbackId = null } = {}) {
  sanitizeManeuverChecked();
  const checked = (state.maneuverChecked || []).map(String).filter(Boolean);
  if (checked.length) {
    setManeuverSelection(checked, { openWorkspace, preserveMapView });
    return;
  }
  if (fallbackId) {
    inspectManeuver(fallbackId, { openWorkspace, preserveMapView });
    return;
  }
  setManeuverSelection([], { openWorkspace: false, preserveMapView });
}

function toggleManeuverCompareSelection(maneuverId, { checked = null, openWorkspace = true, preserveMapView = true } = {}) {
  const id = String(maneuverId || '');
  if (!id) return;
  ...
  setCompareSelectionFromChecked({ openWorkspace, preserveMapView, fallbackId: id });
}
...
function renderManeuverPanel() {
  ...
  const checkMeta = el('maneuver-check-meta');
  if (!list) return;
  ...
  if (title) title.textContent = 'Maneuvers';
  ...
  if (exportBtn) exportBtn.disabled = checkedCount < 2;
  if (processBtn) {
  ...
  const toggleSelectBtn = el('btn-maneuver-toggle-select');
  if (toggleSelectBtn) {
    const visibleIds = visible.map(move => String(move?.id || '')).filter(Boolean);
    const checkedIdsForToggle = new Set((state.maneuverChecked || []).map(String));
    const allVisibleChecked = visibleIds.length > 0 && visibleIds.every(id => checkedIdsForToggle.has(id));
    toggleSelectBtn.disabled = !visibleIds.length;
    toggleSelectBtn.textContent = allVisibleChecked ? 'Deselect All' : 'Select All';
  }
  if (checkMeta) {
    checkMeta.textContent = checkedCount < 2
      ? 'Checked maneuvers appear at the top. Use Process Checked for video/pose data.'
      : `${checkedCount} checked. Use Process Checked for video/pose data, or export an overlay map.`;
  }
  if (meta) {
    meta.textContent = !state.projectId
      ? 'Select a project to inspect auto-detected maneuvers.'
      : `${visible.length} shown${(state.maneuvers || []).length !== visible.length ? ` · ${(state.maneuvers || []).length} total` : ''}`;
  }
  list.innerHTML = '';
  ...
  hint.className = 'mnv-panel-hint';
  hint.textContent = 'Click a row to inspect. Shift-click or use Compare to add/remove maneuvers from the compare set.';
  list.appendChild(hint);
  if (!(state.maneuvers || []).length) {
    const empty = document.createElement('div');
    empty.className = 'mnv-empty';
    empty.textContent = 'No video-backed maneuvers have been detected for this project yet.';
    list.appendChild(empty);
    restoreScroll();
    return;
  }
  if (!visible.length) {
    const empty = document.createElement('div');
    empty.className = 'mnv-empty';
    empty.textContent = 'No maneuvers match the current filters.';
    list.appendChild(empty);
    restoreScroll();
    return;
  }
  ...
  const comparablePair = !!getComparableManeuverSelection();
  const compareColorMap = new Map();
  const comparableSelection = getComparableManeuverSelection();
  ...
  for (const maneuver of rows) {
    ...
    row.className = `mnv-row${selected ? ' is-selected' : ''}`;
    ...
    main.innerHTML = `
      <div class="mnv-row-overline">
        <span class="mnv-row-role">
          <input type="checkbox" class="mnv-row-check" ${checkedIds.has(rowId) ? 'checked' : ''} aria-label="Select for compare" title="Select for compare">
          <span class="mnv-row-color-dot" style="background:${color};"></span>
          ${role ? `<span class="mnv-row-role-badge">${role}</span>` : '<span>Compare</span>'}
        </span>
        <span class="mnv-row-role">
    ...
    compareCopy.textContent = selectedIds.size >= 2 && selected
      ? (comparablePair ? `${role || 'Selected'} in compare workspace` : 'Selected set - compare needs the same maneuver type')
      : (selected ? `${role || 'Selected'} in maneuver workspace` : 'Click to inspect, Shift-click to compare');

    const checkbox = main.querySelector('.mnv-row-check');
    if (checkbox) {
      checkbox.addEventListener('click', evt => evt.stopPropagation());
      checkbox.addEventListener('change', evt => {
        evt.stopPropagation();
        toggleManeuverChecked(maneuver.id, !!evt.target.checked);
      });
    }

    row.append(main, compareCopy);
  ...
  if (checkedVisible.length) {
    makeSectionTitle('Checked maneuvers', checkedVisible.length);
...
function renderManeuverMap({ preserveView = false } = {}) {
  ...
  for (const maneuver of visible) {
    ...
    if (clusterRadiusPx > 0 && !selectedIds.has(String(maneuver.id)) && state.maneuverMap) {
      const markerPoint = state.maneuverMap.latLngToLayerPoint(markerLatLng);
      const overlapsExisting = markerRows.some(row => {
        if (!row?.latlng || selectedIds.has(String(row?.maneuver?.id || ''))) return false;
    ...
    const markerHtml = `
      <div class="maneuver-map-chip-shell">
        <div class="maneuver-map-chip ${String(maneuver.type || '')} ${selectedIds.has(String(maneuver.id)) ? 'selected' : ''}" style="background:${color};">${String(maneuver.type || '').startsWith('j') ? 'J' : 'T'}</div>
    ...
    const role = roleMap.get(String(maneuver.id)) || '';
    marker.bindTooltip(
      `${role ? `${role} · ` : ''}${maneuver?.athlete_name || 'Athlete'} ${getManeuverTypeLabel(maneuver)} · ${fmtClock(maneuver?.anchor_ts)} · ${processState.label}`,
      {
        direction: 'top',
        offset: [0, -20],
        opacity: 0.96,
    ...
    marker.on('click', evt => {
      ...
      if (originalEvent?.shiftKey) {
        toggleManeuverCompareSelection(maneuver.id, {
          checked: !(state.maneuverChecked || []).map(String).includes(String(maneuver.id)),
          openWorkspace: true,
          preserveMapView: true,
        });
      } else {
...
async function triggerManeuverPoseExtraction(maneuvers, allowedVideoIds = null) {
  ...
  for (const move of maneuvers) {
    ...
    if (hasRenderableManeuverPose(cachedAnalysis)) {
      skippedReady++;
      continue;
    }
  ...
  for (const ov of jobs) {
    runSkeletonForVideo(ov.vid.id, { startSec: ov.videoStartSec, endSec: ov.videoEndSec, segmentName: ov.segName })
      .then(async () => {
        await loadAllSkeletonCoverages();
...
function buildManeuverProcessActions({
  ...
  const hasReadyPose = moves.every((move, idx) => (
    !!analysisRows[idx] && hasRenderableManeuverPose(analysisRows[idx])
  ));
...
function getManeuverTimeShift(analysis, key, align = 'anchor') {
  ...
  return ['sog', 'heading', 'rudder', 'trunk', 'com_y'].includes(String(key || '')) ? anchorOffset : 0;
}
...
async function createManeuverVideoCard(label, maneuver, analysis, color, {
  ...
  const overlaps = findManeuverOverlappingVideos(maneuver, { analyzableOnly: true });
  const fallback = overlaps[0] || null;
  const analysisFileId = String(analysis?.mediaRefs?.primaryVideoFileId || '');
  const fileId = overlaps.some(row => String(row?.vid?.id || '') === analysisFileId)
    ? analysisFileId
    : String(fallback?.vid?.id || analysisFileId || '');
  if (!fileId) {
    meta.append(
      title,
      document.createTextNode('No analyzable athlete video is available for this maneuver window.'),
    );
  ...
  try {
    videoEl.src = await getVideoURL(fileId);
  } catch {
  ...
  const startSecRaw = Number(analysis?.mediaRefs?.videoStartSec ?? fallback?.videoStartSec);
  const endSecRaw = Number(analysis?.mediaRefs?.videoEndSec ?? fallback?.videoEndSec);
...
function syncManeuverVideoEntries(entries = []) {
  ...
  const syncFrom = (source, mode = 'seek') => {
    ...
    try {
      const rel = (Number(source.currentTime) || 0) - (Number(sourceEntry.anchorSec) + Number(sourceEntry.offsetSec || 0));
        ...
        if (Math.abs((Number(entry.videoEl.currentTime) || 0) - target) > 0.14) {
  ...
  for (const entry of videos) {
    ...
    entry.videoEl.addEventListener('play', () => syncFrom(entry.videoEl, 'play'));
    entry.videoEl.addEventListener('pause', () => syncFrom(entry.videoEl, 'pause'));
  }
}

function openManeuverComparePlaybackOverlay(entries = []) {
  const sourceEntries = (Array.isArray(entries) ? entries : []).filter(entry => entry?.videoEl);
  if (sourceEntries.length < 2) return;
  const old = document.querySelector('.maneuver-playback-overlay');
  if (old) old.remove();

  const overlay = document.createElement('div');
  overlay.className = 'maneuver-playback-overlay';
  overlay.innerHTML = `
    <div class="maneuver-playback-shell">
      <div class="maneuver-playback-head">
        <div>
          <div class="maneuver-playback-title">Compare playback</div>
          <div class="maneuver-playback-sub">Synchronized side-by-side video. Press Esc or Close to return.</div>
        </div>
        <button class="btn sm" type="button" data-role="close">Close</button>
      </div>
      <div class="maneuver-playback-grid"></div>
    </div>
  `;
  const grid = overlay.querySelector('.maneuver-playback-grid');
  const overlayEntries = [];
  sourceEntries.forEach((entry, idx) => {
    const cell = document.createElement('div');
    cell.className = 'maneuver-playback-cell';
    const video = document.createElement('video');
    video.controls = true;
    video.muted = true;
    video.playsInline = true;
    video.src = entry.videoEl.currentSrc || entry.videoEl.src;
    const title = document.createElement('div');
    title.className = 'maneuver-playback-label';
    title.textContent = entry.label || getManeuverRoleLabel(idx);
    cell.append(video, title);
    ...
    video.addEventListener('loadedmetadata', () => {
      try { video.currentTime = Number(entry.videoEl.currentTime) || Number(entry.anchorSec) || 0; } catch {}
    }, { once: true });
  });
  syncManeuverVideoEntries(overlayEntries);
  const close = () => {
    try {
      for (const entry of overlayEntries) entry.videoEl.pause();
    } catch {}
  ...
  if (requestFullscreen) {
    try { requestFullscreen.call(overlay); } catch {}
  }
}
...
function buildManeuverSelectionState() {
  ...
  if (selected.length === 0) {
    return { kind: 'empty', selected, comparable: null, reason: 'Select a maneuver from the map or the Maneuvers panel. Shift-click or check same-type maneuvers to compare.' };
  }
...
async function ensureManeuverAnalysisReady(maneuver, { force = false } = {}) {
  ...
  const cachedReady = !!cached
    ...
    && (!bestVideo?.vid || hasRenderableManeuverPose(cached));
  ...
  if (bestVideo?.vid) {
    ...
    if (!poseCoverageReady && !poseProcessingActive) {
      try {
...
async function buildSingleManeuverContent(maneuver, analysis) {
  ...
  statsGrid.append(
    buildManeuverStatsBlock('Before', maneuver?.preStats),
    buildManeuverStatsBlock('During', maneuver?.duringStats),
    buildManeuverStatsBlock('After', maneuver?.postStats),
  );
...
async function buildCompareManeuverContent(maneuvers, analyses) {
  ...
  const heatmapJobs = [];
  const overlayGeometries = await Promise.all(maneuvers.map((move, idx) => resolveManeuverOverlayGeometry(move, analyses[idx])));

  const mapSec = createManeuverSection(`${getManeuverTypeLabel(maneuvers[0])} Compare`, 'Shift-click or check maneuvers of the same type to compare them here.');
  const compareBriefs = document.createElement('div');
  compareBriefs.className = 'maneuver-compare-grid';
  maneuvers.forEach((move, idx) => {
    compareBriefs.appendChild(buildManeuverBrief(move, {
      color: colors[idx],
      prefix: getManeuverRoleLabel(idx),
    }));
  });
  const mapCard = document.createElement('div');
  mapCard.className = 'maneuver-map-surface maneuver-compare-fixed-card';
  const mapCanvas = createManeuverCanvas(1160, 600);
  drawManeuverOverlayMap(mapCanvas, maneuvers.map((move, idx) => ({
    color: colors[idx],
    ...
    title: 'Normalized compare overlay',
    paddingRatioX: 0.2,
    paddingRatioY: 0.22,
    arrowSpread: true,
  });
  ...
  mapSec.body.append(compareBriefs, mapCard);
  fragment.appendChild(mapSec.section);

  const mediaSec = createManeuverSection('Video + Pose Density', 'Each selected maneuver keeps its own video and boat-referenced pose density view.');
  const mediaGrid = document.createElement('div');
  mediaGrid.className = 'maneuver-compare-media-grid';
  const videoEntries = [];
  for (let i = 0; i < maneuvers.length; i++) {
    const column = document.createElement('div');
    column.className = 'maneuver-compare-column';
    column.appendChild(buildManeuverBrief(maneuvers[i], {
      color: colors[i],
      prefix: getManeuverRoleLabel(i),
    }));

    const entry = await createManeuverVideoCard(`${getManeuverRoleLabel(i)} video`, maneuvers[i], analyses[i], colors[i], {
      showOffsetSlider: i > 0,
      initialOffsetSec: i > 0 ? getManeuverCompareOffsetSec(maneuvers, maneuvers[i]) : 0,
      onOffsetChange: (offsetSec, committed) => {
        setManeuverCompareOffsetSec(maneuvers, maneuvers[i], offsetSec);
        if (committed) {
    ...
    const clouds = getManeuverPosePointClouds(analyses[i]);
    const poseBusy = isManeuverPoseRunActive(analyses[i]?.mediaRefs?.primaryVideoFileId);
    const heatmapGrid = document.createElement('div');
    heatmapGrid.className = 'maneuver-heatmap-grid';
    heatmapGrid.append(
      createManeuverHeatmapTile(
        'Athlete movement density',
        clouds.keypointXY.length
          ? `${clouds.frameCount || getManeuverPoseFrames(analyses[i]).length} pose frames projected into boat space`
          : (poseBusy ? 'Pose processing is still running for this maneuver window.' : 'Pose frames were unavailable for this maneuver.'),
        createManeuverHeatmapJob(clouds.keypointXY, colors[i], 'keypoints'),
        heatmapJobs,
      ),
      createManeuverHeatmapTile(
        'Center of mass density',
        clouds.comXY.length
          ? 'Center-of-mass positions over the full maneuver window'
          : (poseBusy ? 'Center-of-mass data will appear when pose processing finishes.' : 'Center-of-mass data was unavailable for this maneuver.'),
        createManeuverHeatmapJob(clouds.comXY, colors[i], 'com'),
        heatmapJobs,
      ),
    );
    column.appendChild(heatmapGrid);
    mediaGrid.appendChild(column);
  }
  syncManeuverVideoEntries(videoEntries);
  if (videoEntries.filter(entry => entry?.videoEl).length >= 2) {
    const fullBtn = document.createElement('button');
    fullBtn.className = 'btn sm maneuver-fullscreen-compare-btn';
    fullBtn.type = 'button';
    fullBtn.textContent = 'Fullscreen side-by-side playback';
    fullBtn.onclick = () => openManeuverComparePlaybackOverlay(videoEntries);
    mediaSec.body.appendChild(fullBtn);
  }
  mediaSec.body.appendChild(mediaGrid);
  fragment.appendChild(mediaSec.section);

  const summarySec = createManeuverSection('Summary Stats', 'Before, during, and after metrics for the selected maneuvers.');
  const summaryGrid = document.createElement('div');
  summaryGrid.className = 'maneuver-compare-summary';
  maneuvers.forEach((move, idx) => {
    const card = document.createElement('div');
    card.className = 'maneuver-chart-card';
    const brief = buildManeuverBrief(move, {
      color: colors[idx],
      prefix: getManeuverRoleLabel(idx),
    });
    const statsGrid = document.createElement('div');
    statsGrid.className = 'maneuver-stats-table';
    statsGrid.append(
      buildManeuverStatsBlock('Before', move?.preStats),
      buildManeuverStatsBlock('During', move?.duringStats),
      buildManeuverStatsBlock('After', move?.postStats),
    );
    card.append(brief, statsGrid);
    summaryGrid.appendChild(card);
  });
  summarySec.body.appendChild(summaryGrid);
  fragment.appendChild(summarySec.section);

  const chartSec = createManeuverSection('Compare Timelines', 'Only timelines with usable data are shown.');
  const chartGrid = document.createElement('div');
  chartGrid.className = 'maneuver-chart-grid maneuver-compare-chart-grid';
  const chartDefs = [
    { key: 'sog', title: 'Speed', yLabel: 'SOG (kt)', baselineAt: null },
    { key: 'vmg', title: 'VMG', yLabel: 'VMG (kt)', baselineAt: 0 },
    { key: 'twa', title: 'TWA Crossing', yLabel: 'Signed TWA (deg)', baselineAt: 0 },
  ...
  if (!visibleChartDefs.length) {
    chartSec.body.appendChild(createManeuverInlineNote('These maneuvers did not have enough synchronized telemetry to draw readable comparison timelines.'));
  }
  for (const def of visibleChartDefs) {
    const card = document.createElement('div');
    card.className = 'maneuver-chart-card';
    ...
    title.className = 'maneuver-chart-title';
    title.textContent = def.title;
    const canvas = createManeuverCanvas(960, 360);
...
async function downloadCheckedManeuverCompareMap() {
  ...
  for (const move of checked) {
    ...
    if (!analysis || !hasRenderableManeuverGpsOverlay(analysis) || !hasRenderableManeuverPose(analysis)) {
      analysis = await ensureManeuverAnalysisReady(move, { force: false }).catch(() => null);
    }
    analyses.push(analysis || null);
  }
...
function getSegmentProcessingStatus(seg) {
  ...
  for (const overlap of analyzableOverlapping) {
    ...
    const processedRanges = state.cvStatuses?.[overlap.vid.id]?.processed_ranges;
    const liveCoverage = state.skeletonCoverage[overlap.vid.id] || [];
    const coverage = [
...
function renderWindMapControl() {
  ...
  if (!state.advancedMode || !csvCount) {
  ...
  const towFilteringDisabled = !!state.windConfig?.disableTowFilteringTrusted;
  const chipText = state.wind.loading
    ? `Estimating ${resolvedCount}/${csvCount}`
    : `${resolvedCount}/${csvCount} CSV`;
  ...
  root.innerHTML = `
    <div class="wind-card ${state.wind.loading ? 'is-loading' : ''}">
      <div class="wind-card-head">
        ...
        <div class="wind-card-chip">${chipText}</div>
      </div>
      <div class="wind-card-body">
        ...
        <div class="wind-card-metrics">
          ${hasSession ? `
            ...
            <div class="wind-card-sub">${session.speedKts.toFixed(1)} kt likely</div>
            <div class="wind-card-row">
              <span class="wind-card-row-key">Tow filter</span>
              <span class="wind-card-row-val">${towFilteringDisabled ? 'Disabled (trusted session)' : 'Enabled'}</span>
            </div>
            <div class="wind-card-local-row">
              ...
              <div class="wind-card-row">
                <span class="wind-card-row-key">Playhead</span>
...
async function autoAssignCsvAthletesFromMemory() {
  ...
  for (const csv of csvs) {
    ...
    } else if (!athlete.color && profile.color) {
...
function shouldFollowPlaybackOnMap() {
  if (!state.map || state.mapBaseZoom == null) return false;
  return state.map.getZoom() > (state.mapBaseZoom + 0.5);
}
...
function getVideoMapPoints(video, csvTracks = []) {
  ...
  for (const csv of candidates) {
    const points = Array.isArray(csv?.points) ? csv.points : [];
    ...
    if (Number.isFinite(start) && Number.isFinite(end)) {
      const ranged = valid.filter(point => Number(point.ts) >= start && Number(point.ts) <= end);
      if (ranged.length >= 2) sliced = ranged;
    }
...
function renderMap() {
  ...
  for(const csv of csvs) {
    ...
    const clickH = makeClickHandler(csv.points, csv);
  ...
  if (!heavyProject) {
    ...
    const csvTrackEntries = csvs.map(c => ({id: c.id, points: c.points || []})).filter(e => e.points.length);
...
function showNoVideo(show) {
  ...
  if (subtitle) {
    subtitle.textContent = playbackCount > 0
      ? `Use the Phone/External toggle in timeline controls to arm synced playback.${timelineSlotCount > 0 ? ' CSV speed and live stats still run below.' : ''}`
      : (timelineSlotCount > 0
        ? 'CSV speed and live stats are still available in the timeline below.'
        : '');
  }
  if(nv) nv.style.display = show ? 'flex' : 'none';
  if(vg) vg.style.display = show ? 'none' : 'flex';
}
...
function buildTimeline() {
  ...
  for(const [athId, avids] of athleteVids) {
    ...
    // Collect SOG data from CSV matched tracks + video points
    const sogSourcePts = collectSogForAthlete(athId, avids);
    const windCsvIds = collectSlotWindCsvIds(avids);
    ...
    slots.push({
      ...
      statsSeries: {
        ...
        motionDir: buildNumericSeries(collectMotionDirectionForAthlete(avids), 'motionDir', TIMELINE_TRACK_SERIES_OPTS),
        heading: buildNumericSeries(collectTrackMetricForAthlete(avids, 'hdg'), 'hdg', TIMELINE_TRACK_SERIES_OPTS),
        cog: buildNumericSeries(collectTrackMetricForAthlete(avids, 'cog'), 'cog', TIMELINE_TRACK_SERIES_OPTS),
        heel: buildNumericSeries(collectTrackMetricForAthlete(avids, 'heel'), 'heel', TIMELINE_TRACK_SERIES_OPTS),
        pitch: buildNumericSeries(collectTrackMetricForAthlete(avids, 'trim'), 'trim', TIMELINE_TRACK_SERIES_OPTS),
  ...
  for(const slot of slots) {
    ...
    paneBody.appendChild(heatmapCol);
...
function collectSogForAthlete(athId, avids) {
  const csvPts = [];
  const videoPts = [];
  const csvs = state.mapData?.csvs || [];
  ...
  const derived = [];
  for (const v of avids) derived.push(...buildDerivedSogFromGps(v.points || []));
...
function collectSogForVideo(v) {
  ...
  if (csvPts.length) return finalizeSogPoints(csvPts);
  if (videoPts.length) return finalizeSogPoints(videoPts);
  return finalizeSogPoints(buildDerivedSogFromGps(v.points || []));
}
...
function collectTrackMetricForAthlete(avids, metricKey) {
  const csvPts = [];
  const videoPts = [];
  const csvs = state.mapData?.csvs || [];
  const seenCsvIds = new Set();
  ...
  for (const v of avids || []) {
    if (v?.best_match_csv_id) seenCsvIds.add(v.best_match_csv_id);
    for (const p of (v?.points || [])) {
      if (p?.ts != null && p?.[metricKey] != null) {
        videoPts.push({ ts: p.ts, [metricKey]: p[metricKey] });
      }
    }
  }
  ...
  for (const csvId of seenCsvIds) {
    ...
    for (const p of (csv?.points || [])) {
      if (p?.ts != null && p?.[metricKey] != null) {
        csvPts.push({ ts: p.ts, [metricKey]: p[metricKey] });
      }
    }
  }
...
function collectMotionDirectionForAthlete(avids) {
  const csvPts = [];
  const videoPts = [];
  const csvs = state.mapData?.csvs || [];
  const seenCsvIds = new Set();
...
function collectSlotWindCsvIds(videos) {
  const out = [];
  const seen = new Set();
...
const TIMELINE_STAT_DEFS = Object.freeze([
  ...
  { key: 'cog', label: 'COG', suffix: '\u00B0', seriesKey: 'cog', overlayLabel: 'COG' },
...
function createEmptyProcessedTimelineMetricSet() {
  return { trunk_angle: [], roll_moment: [], rudder_angle: [] };
}

function createEmptyProcessedTimelineStatValues() {
  return { roll: null, trunk: null, rudder: null };
}
...
function buildVideoSecToAbsTsConverter(video) {
  if (!video) return () => null;
  if (!Array.isArray(video.points) || video.points.length === 0) {
...
async function loadProcessedTimelineMetricPoints(projectId, video, loadToken = _timelineMetricLoadToken) {
  ...
  const loadPromise = (async () => {
    ...
    abortTimelineMetricLoadIfNeeded(loadToken);
    entry.ready = true;
    return { trunk_angle: trunk, roll_moment: roll, rudder_angle: rudder };
  })().catch((err) => {
...
function populateTimelineStats() {
  ...
  for (const slot of slots) {
    ...
    metrics.className = 'tl-ath-metrics';

    const makeMetric = (def) => {
      const metric = document.createElement('button');
      metric.type = 'button';
      metric.className = 'tl-ath-metric';
      metric.title = `${def.title || def.label}. Click to toggle the overlay in all video panes.`;
      const key = document.createElement('span');
      ...
      metric.append(key, value);
      metrics.appendChild(metric);
      return { button: metric, value };
    };

    const metricRefs = {};
    for (const def of TIMELINE_STAT_DEFS) {
      metricRefs[def.key] = makeMetric(def);
    }
...
function updateTimelineStats() {
  ...
  wrap.title = `SOG / Heel / Pitch use a centered ${windowSec}s rolling mean; Heading / COG / Roll M / Trunk / Rudder show nearest available sample; VMG shows absolute value and TWA uses current motion direction against the estimated wind`;
  ...
  for (const slot of slots) {
    ...
    const statValues = {
      sog: computeRollingMean(slot.statsSeries?.sog, currentTs, windowSec),
      ...
      heel: computeRollingMean(slot.statsSeries?.heel, currentTs, windowSec),
      pitch: computeRollingMean(slot.statsSeries?.pitch, currentTs, windowSec),
      ...processedStatValues,
    };
...
function tlSeekTo(absTs) {
  ...
  for(const slot of tl.athleteSlots) {
    ...
    if(vid) {
      ...
      // Move map marker
      if(videoSec != null) movePositionMarker(fileId, videoSec);
    } else {
...
function tlAnimLoop(now) {
  ...
  for(const slot of tl.athleteSlots) {
    ...
    if(vid) {
      ...
      if(videoSec2 != null) movePositionMarker(vid.id, videoSec2);
    } else {
      if (slot.videos.length > 0) showSlotEmptyState(slot);
      else if (slot.paneEl) slot.paneEl.style.display = 'none';
      if (slot.currentFileId || slot._desiredFileId) stopSlotPlayback(slot, !USE_IPAD_VIDEO_WORKAROUNDS);
...
function initSogScrub() {
  ...
  wrap.addEventListener('pointerdown', e => {
    // Shift+click or segment mode active → segment creation on scrub bar
    if(state.segmentSelect.active || e.shiftKey) {
      const ts = tsFromEvent(e);
      if(ts != null) {
...
function openAssignPopup(fileId, anchor, kind) {
  ...
  noneOpt.onclick = async()=>{
    if (kind === 'video') {
      ...
      await setFileMeta(fileId, { athlete_id: null });
    }
  ...
  for(const a of state.athletes){
    ...
    opt.onclick = async()=>{
      if (kind === 'video') {
        ...
        await setFileMeta(fileId, { athlete_id: a.id });
      }
      closePopup(); renderFilesList(); buildTimeline(); void refreshManeuvers('assignment-updated');
    };
...
async function saveAthlete() {
  ...
  if(state.editingAthleteId) {
    ...
    state.athletes.push({id: DB.uuid(), name, weight});
  }
  await saveAthletes();
  renderAthletes();
  renderFilesList();
...
function getSetupState() {
  ...
  return {
    ...
    steps: {
      1: hasProject,
      2: hasFiles && !state.uploadInProgress,
      3: hasAthletes,
      4: hasFiles && assignmentsReady,
      5: hasProject && hasFiles && hasAthletes && assignmentsReady && !state.uploadInProgress,
    },
  };
}
...
function renderWizardStatus() {
  const setup = getSetupState();
  const currentStep = Math.max(1, Math.min(5, state.wizardStep || 1));
  document.querySelectorAll('.wiz-step').forEach(stepEl => {
    ...
    if (stepNum === 5 && currentStep === 5 && !setup.ready) stepEl.classList.add('warn');
  });
...
function syncWizardFooterButtons(step = state.wizardStep) {
  ...
  nextBtn.disabled =
    ...
    (currentStep === 2 && (!setup.hasFiles || state.uploadInProgress)) ||
    (currentStep === 3 && !setup.hasAthletes) ||
    (currentStep === 4 && !setup.assignmentsReady);
  nextBtn.innerHTML = currentStep === 4
    ? 'Finish <svg class="icon-svg" style="width:14px;height:14px"><use href="#ico-skip-fwd"/></svg>'
    : 'Next <svg class="icon-svg" style="width:14px;height:14px"><use href="#ico-skip-fwd"/></svg>';
}

function updateSetupUiState() {
  syncWizardFooterButtons();
  renderWizardStatus();
  if (state.wizardStep === 5) renderReadySummary();
}
...
function wizGoTo(step) {
  ...
  if(step === 2) renderFilesList();
  if(step === 3) renderAthletes();
  if(step === 4) renderAssignList();
  if(step === 5) renderReadySummary();
  renderWizardStatus();
}

function wizNext() {
  if(state.wizardStep < 5) wizGoTo(state.wizardStep + 1);
}
function wizBack() {
...
function renderAssignList() {
  ...
  if(csvs.length === 0 && videos.length === 0) {
    list.innerHTML = '<div class="empty-state" style="padding:20px;"><div>No files uploaded yet. Go back to Step 2.</div></div>';
    return;
  }
  ...
  if(csvs.length) {
    ...
    for(const f of csvs) {
      ...
      sel.onchange = async () => {
        ...
        await setFileMeta(f.id, {athlete_id: athId});
        renderAssignList();
        buildTimeline();
        void refreshManeuvers('assignment-updated');
      };
...
function sameSegmentRunRequest(a, b, tolerance = 0.35) {
  ...
  return Math.abs(aStart - bStart) <= tolerance && sameEnd;
}
...
async function runSkeletonForAllVideos() {
  ...
  try {
    ...
    if (state.segments.length > 0) {
      ...
      if (totalJobs === 0) { alert('No analyzable videos overlap any segments.'); return; }
      ...
      console.log(`[Skeleton] processing ${totalJobs} video-segment jobs across ${state.segments.length} segments`);
      ...
      renderAnalysisTab();
      drawSogCanvas();
      alert(`Skeleton processing complete: ${done}/${totalJobs} jobs across ${state.segments.length} segment(s).`);

    } else {
      // No segments: process all videos at full length (fallback)
      const files = await DB.listFiles(state.projectId);
      const videos = files.filter(f => f.kind === 'video' && !f.playback_only);
      if (videos.length === 0) { alert('No analyzable video files in project.'); return; }
      ...
      renderAnalysisTab();
      drawSogCanvas();
      alert(`Skeleton processing complete: ${done}/${videos.length} videos processed.`);
    }
  } catch(e) {
    console.error('Skeleton processing failed:', e);
    alert('Skeleton processing failed: ' + e.message);
...
async function generateReportAll() {
  ...
  try {
    ...
    const polarPlotsEnabled = !!state.advancedMode && !!state.reportOptions?.polarPlots;
    const maneuverAnalysisEnabled = !!state.advancedMode && !!state.reportOptions?.maneuverAnalysis;
...
function applyReportOptionsVisibility() {
  ...
  if (wrap) wrap.style.display = state.advancedMode ? 'block' : 'none';
  const polarToggle = el('report-opt-polar-plots');
  if (polarToggle) {
...
function applyAdvancedModeVisibility() {
  ...
  if (stlBtn) stlBtn.style.display = advanced ? '' : 'none';
...
async function registerServiceWorker() {
  ...
  try {
    const registration = await navigator.serviceWorker.register(
      new URL('./sw.js?v=20260306tlstats1', import.meta.url).toString(),
      { updateViaCache: 'none' },
    );
...
async function init() {
  ...
  const toggleSelectBtn = el('btn-maneuver-toggle-select');
  if (toggleSelectBtn) toggleSelectBtn.onclick = () => toggleSelectVisibleManeuvers();
...
function buildInlineHeatmapStatsCard(result, color, plotJobs, vmgContext = null) {
  ...
  card.appendChild(createInlineHeatmapVmgCard(result, color, vmgContext));
...
function renderInlineHeatmapsForResults(seg, results, loadToken) {
  ...
  const seenVmgKeys = new Set();
  for (const slot of state.tl?.athleteSlots || []) {
    if (!slot?._heatmapColEl) continue;
    if (!slot?.paneEl?.classList.contains('show-heatmaps')) continue;
    if (slot.paneEl.style.display === 'none') continue;
    const slotResult = findHeatmapResultForSlot(slot, results);
    if (!slotResult) continue;
    const rowKey = buildInlineHeatmapVmgKey(slotResult);
    if (seenVmgKeys.has(rowKey)) continue;
    seenVmgKeys.add(rowKey);
    visibleResults.push(slotResult);
  }
  const vmgContext = buildInlineHeatmapVmgContext(visibleResults, seg);
  ...
  for (const slot of state.tl?.athleteSlots || []) {
    ...
    slot._heatmapColEl.appendChild(buildInlineHeatmapStatsCard(result, color, plotJobs, vmgContext));
  }
  queueInlineHeatmapScrollSync();
...
async function syncInlineHeatmapsToCurrentSegment(seg = (isAnalysisViewActive() ? getSegmentAtTs(state.tl?.currentTs) : null), inAnalysis = isAnalysisViewActive()) {
  ...
  try {
    renderInlineHeatmapLoading(seg, 'Estimating wind-driven VMG...', 0.02);
    await ensureWindEstimatesReady();
    const reportData = await buildReportData(
      state.projectId,
      [segId],
...
async function openHeatmapViewer({ forceReload = false, silentNoSegment = false } = {}) {
  ...
  try {
    renderHeatmapLoadingState(seg, 'Estimating wind-driven VMG...', 0.02);
    await ensureWindEstimatesReady();
    const reportData = await buildReportData(
      state.projectId,
      [String(seg.id)],
